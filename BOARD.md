...
| E-079 | Replay of every alerted listing as a dated ledger | in_progress | high | `index_cli.py housing-replay [--dry-run]`: re-judges each alerted unit against the active revision, queues `kind='replay'` rows (posted-at + first-alert date in the report) behind a self-identifying header; dedup on (unit, verdict, replay). Run 2026-09-05 05:45 UTC: 7 of 15 forwarded into the DM, then Telegram answered PEER_FLOOD (17 messages in 52 s to a peer that had never written back) and the remaining 7 fell back to the bot with a link (`forward_status=bot_fallback`), one report lost its deleted original. The owner's reply did NOT lift the limit (PEER_FLOOD again at 06:12, seven duplicate bot reports); recovery is now probe-driven: the first send that reaches the DM after a pause reopens every bot_fallback/unavailable row, and a row that already went through the bot waits instead of repeating it (`bot_reports`). Pause expires 12:12 UTC; by hand: `housing-replay --redo-fallback` |
